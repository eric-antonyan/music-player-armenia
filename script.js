...
let speedsButton = document.querySelectorAll('.speedsButton');
...
function loadSong(songList) {
    ...
    singerName.textContent = songList.singer;
    ...
    category.textContent = songList.category;
...
let categButtons = document.querySelectorAll('.categButtons');
let contSongs = document.querySelector('.contSongs');

categButtons.forEach((select, i) => {
    select.onclick = () => {
        contSongs.innerHTML = ''
        let fil = songList.filter(item => {
            if (select.getAttribute('data') == item.category) {
                return item
            }
        })
        fil.forEach(($, i) => {
            ...
            contSongs.append(div);
            div.append(h2, h3);
            h2.textContent = $.name;
            h3.textContent = $.singer;
            div.onclick = () => {
                loadSong(fil[i]);
                audioPlay();
                playPause.className = 'fa fa-pause';
